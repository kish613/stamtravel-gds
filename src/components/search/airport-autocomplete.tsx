...
export function AirportAutocomplete({ label, value, onChange }: Props) {
  ...
  return (
    <div className="relative">
      ...
      />
      <input type="hidden" value={value} readOnly onChange={() => undefined} aria-hidden />
      {open && (
        <Card className="absolute left-0 right-0 z-20 max-h-60 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="px-2 py-2 text-[12px] text-muted-foreground">No airport match.</div>
          ) : (
            <ul>
              {filtered.map((a) => (
                <li key={a.code}>
                  <button
                    type="button"
                    className="w-full px-2 py-1.5 text-left text-[12px] hover:bg-muted transition-colors"
                    onMouseDown={() => {
                  ...
                  >
                    <div className="font-medium text-foreground">{a.code} · {a.city}</div>
