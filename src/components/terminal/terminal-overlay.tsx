...
import { executeTerminalCommand, TERMINAL_COMMANDS } from '@/lib/commands/commands';
...
export function TerminalPanel() {
  ...
  useEffect(() => {
    ...
    const create = async () => {
      ...
      terminal.focus();
      terminal.writeln('Sabre command console ready. Type HLP for commands or CAPS for capability status.');
      terminal.writeln('Classic-air workflow is active; hotel and car booking remain disabled in v1.');
      ...
      const onData = terminal.onData((data) => {
        if (data === '\r') {
          ...
          void executeTerminalCommand(command)
            ...
            .catch((error) => {
              terminal.writeln(error instanceof Error ? error.message : 'Command execution failed.');
            })
            .finally(() => {
              terminal.write(`${PROMPT} `);
          ...
          if (command) {
            setHistory((prev) => {
              const next = [command, ...prev.filter((entry) => entry !== command)].slice(0, 50);
        ...
        if (data === '\u001b[A') {
          const nextIndex = Math.min(historyRef.current.length - 1, commandHistoryIndexRef.current + 1);
        ...
        if (data === '\u001b[B') {
          const nextIndex = Math.max(-1, commandHistoryIndexRef.current - 1);
          commandHistoryIndexRef.current = nextIndex;
          setCommandHistoryIndex(nextIndex);
          ...
          {isMinimized ? (
            ...
            >
              ...
              >
                ...
                <span className="font-semibold text-sm whitespace-nowrap">Sabre Console</span>
              </motion.button>
            </motion.div>
          ) : (
